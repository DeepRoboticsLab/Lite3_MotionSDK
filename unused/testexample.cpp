...
// void TestExample::PreStandUp(RobotCmd &cmd, double time,RobotData &data_state) {
...
/// @param time Current timestamp
/// @param data_state Real-time status data of robot
void TestExample::Motion(RobotCmd &cmd, double time,RobotData &data_state) {
  double motion_time = 1.5;
  double cycle_time = 0.001;
...
/// @param time Current timestamp
void TestExample::GetInitData(LegData data, double time) {
  init_time = time;
...
void TestExample::SwingToAngle(Vec3 initial_angle, Vec3 final_angle,
  ...
  if(true){
    cmd.joint_cmd[3 * leg_side].kp = 45;
    cmd.joint_cmd[3 * leg_side + 1].kp = 45;
    cmd.joint_cmd[3 * leg_side + 2].kp = 45;
...
void TestExample::CubicSpline(double init_position, double init_velocity,
