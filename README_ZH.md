...
- 如果通过WiFi连接，请查看开发主机分配到的IP：
	...
	- 如果网段为1，则运动主机地址为**192.168.1.120**，ssh登录用户名为`firefly`，密码为 `firefly`。

	- 如果网段为2，则运动主机地址为**192.168.2.1**，ssh登录用户名为`user`，密码为 `123456`。 

- 如果通过背部网口连接，则运动主机地址为**192.168.1.120**，请在开发主机上打开终端，输入`ssh user@192.168.1.120`，密码为 `123456`，尝试远程连接运动主机：

	- 如果成功，则用户名为`user`，密码为`123456`；否则用户名为`firefly`，密码为 `firefly`。
...
- 首先检查开发主机是否与机器狗主机处于同一网段下（如果是在机器狗上运行SDK，此步骤可跳过）：将开发主机连到接机器狗的WiFi网络或背部网口，然后在开发主机上ping运动主机（运动主机地址参考第3节），ping通之后ssh连接到机器狗运动主机内，在运动主机内ping开发主机的静态IP。如果无法ping通，请尝试手动设置自己开发主机的IP地址，并再次按照第5节对配置文件进行修改。
